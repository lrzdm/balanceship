...
def cached_all_tickers():
    return read_all_tickers()

def load_ticker_bar_data():
    tickers = read_all_tickers()
    sampled = random.sample(tickers, min(25, len(tickers)))
...
logo1 = get_base64("images/logo1.png")
logo2 = get_base64("images/logo2.png")
map_base64 = get_base64("images/Map_Chart.png")

# ----------------- Dati KPI sintetici per le card -----------------
n_companies = len(read_all_tickers())
n_years = 4
...
# ----------------- HTML/CSS: Navbar, Ticker, Video BG "safe" -----------------
bar_items = load_ticker_bar_data()  # usato sotto

navbar_ticker_video = f"""
<style>
  @import url('https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;600;700&display=swap');

  :root {{
    --nav-h: 70px;      /* altezza navbar */
    --ticker-h: 40px;   /* altezza ticker */
    --top-ui: calc(var(--nav-h) + var(--ticker-h));
  }}
  ...
  /* VIDEO BACKGROUND "SAFE" */
  .video-background {{
    position: fixed;
    top: var(--top-ui);
    left: 0;
    width: 100vw;
    height: calc(100vh - var(--top-ui));
    object-fit: cover;
    z-index: -1;
    opacity: 0.8;
    background-color: black;
  ...
  /* Supporto viewport dinamici */
  @supports (height: 100dvh) {{
    .video-background {{ height: calc(100dvh - var(--top-ui)); }}
  }}
  @supports (height: 100svh) {{
    .video-background {{ height: calc(100svh - var(--top-ui)); }}
  }}
...
<video autoplay muted loop playsinline poster="https://balanceship.net/images/video_poster.jpg" class="video-background">
...
<div class="navbar">
  <div class="navbar-left">
    <img src="data:image/png;base64,{logo1}" alt="logo1"/>
    <img src="data:image/png;base64,{logo2}" alt="logo2"/>
  </div>
...
# Duplica gli item per lo scorrimento continuo
for t, y, val in (bar_items * 2):
    navbar_ticker_video += f'<span class="ticker-item" style="color:{get_random_color()};">{t} ({y}): {val}</span>'
...
# Rende il componente HTML compatto (navbar+ticker+video BG non necessitano altezza grande)
html(navbar_ticker_video, height=140)

# ----------------- Offset contenuti sotto top UI -----------------
# Spinge il contenuto sotto navbar+ticker (70+40 = 110 → arrotondiamo a 120)
st.markdown("<div style='height: 120px;'></div>", unsafe_allow_html=True)

# ----------------- HEADLINE -----------------
st.markdown("""
<div style='margin-top: 40px; margin-bottom: 100px; color:#0173C4; text-align:center;'>
...
.profile-card { width: 220px; height: 250px; perspective: 1000px; }
.profile-inner {
...
.profile-front { background-color: #0173C4; color: white; padding-top: 1.2rem; }
.profile-front i { font-size: 3rem; margin-bottom: 0.6rem; display: block; }
.profile-front h4 { font-size: 1.2rem; margin: 0; font-weight: 600; line-height: 1.3; text-align: center; }
.profile-back {
    background-color: #01c4a7; color: white; transform: rotateY(180deg);
    font-size: 2.2rem; font-weight: bold; display: flex; justify-content: center; align-items: center; padding: 1rem;
}
</style>
""", unsafe_allow_html=True)
...
<div class='profile-grid'>
  <div class='profile-card'>
    <div class='profile-inner'>
      ...
      <div class='profile-back'>{n_companies_fmt}</div>
    </div>
  </div>
  <div class='profile-card'>
    <div class='profile-inner'>
      ...
      <div class='profile-back'>{n_records_fmt}</div>
    </div>
  </div>
  <div class='profile-card'>
    <div class='profile-inner'>
      ...
      <div class='profile-back'>{n_years_fmt}</div>
    </div>
  </div>
  <div class='profile-card'>
    <div class='profile-inner'>
      ...
      <div class='profile-back'>{stock_exchanges_fmt}</div>
    </div>
  </div>
</div>
"""
...
<div class='map-box'>
  ...
  <img src="data:image/png;base64,{map_base64}" class="map-img" style="max-width: 100%; height: auto; display: block; margin: 0 auto;"/>
</div>
""", unsafe_allow_html=True)
...
st.sidebar.markdown(f"""
    <div style='text-align: center;'>
        ...
        <a href='https://www.linkedin.com/company/balanceship/' target='_blank' style="display: inline-block; margin-left: 12px;'>
